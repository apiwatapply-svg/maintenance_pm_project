...
const getDedupedSubItems = (rawSubItems: any[]) => {
  if (!rawSubItems || !Array.isArray(rawSubItems)) return [];
...
const validateDropdownValue = (val: string, optionsList: any[], minVal?: number, maxVal?: number) => {
  if (!val) return true;
  const selectedOpt = optionsList.find((o: any) => (typeof o === 'string' ? o : o.label) === val);
  if (selectedOpt && typeof selectedOpt !== 'string' && selectedOpt.spec && typeof selectedOpt.spec === 'string') {
    const hasMinMax = minVal !== null && minVal !== undefined && maxVal !== null && maxVal !== undefined;
    if (hasMinMax) {
      const specStr = selectedOpt.spec.trim();
      if (specStr.includes('-')) {
        const specParts = specStr.split('-').map((s: string) => parseFloat(s.trim()));
        if (specParts.length === 2 && !isNaN(specParts[0]) && !isNaN(specParts[1])) {
          return specParts[0] >= minVal! && specParts[1] <= maxVal!;
        }
      } else {
        const specVal = parseFloat(specStr);
        if (!isNaN(specVal)) {
          return specVal >= minVal! && specVal <= maxVal!;
        }
      }
    }
  }
...
export default function InspectionForm() {
          ...
          sourceChecklists.forEach((c: any) => {
            if (c.options) {
              try {
                ...
                if (opts.subItems && Array.isArray(opts.subItems)) {
                  const deduped = getDedupedSubItems(opts.subItems);
                  deduped.forEach((subItem: any) => {
  ...
  const handleDetailChange = (index: number, field: string, value: any) => {
    ...
    newDetails.forEach((d, idx) => {
      ...
      // [FIX] Validate DROPDOWN with spec (range or single value) against checklist minVal/maxVal
      if (checklist?.type === 'DROPDOWN' && d.value) {
        ...
        if (selectedOpt && typeof selectedOpt !== 'string' && selectedOpt.spec && typeof selectedOpt.spec === 'string') {
          const hasMinMax = checklist.minVal !== null && checklist.minVal !== undefined && checklist.maxVal !== null && checklist.maxVal !== undefined;
          if (hasMinMax) {
            const specStr = selectedOpt.spec.trim();
            if (specStr.includes('-')) {
              // Range spec (e.g. "10-20")
              const specParts = specStr.split('-').map((s: string) => parseFloat(s.trim()));
              if (specParts.length === 2 && !isNaN(specParts[0]) && !isNaN(specParts[1])) {
                d.isPass = specParts[0] >= checklist.minVal && specParts[1] <= checklist.maxVal;
              }
            } else {
              // Single value spec (e.g. "5")
              const specVal = parseFloat(specStr);
              if (!isNaN(specVal)) {
                d.isPass = specVal >= checklist.minVal && specVal <= checklist.maxVal;
              }
            }
          }
        }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    currentChecklists.forEach((checklist: any, index: number) => {
      ...
      let opts: any = {};
      try {
        opts = checklist.options ? JSON.parse(checklist.options) : {};
        if (opts.subItems && opts.subItems.length > 0) isSubItem = true;
      } catch { /* ignore */ }

      if (checklist.isRequired) {
        if (isSubItem) {
          // Validate all sub-items have values
          const deduped = getDedupedSubItems(opts.subItems);
          for (let i = 0; i < deduped.length; i++) {
            const subItem = deduped[i];
            const name = typeof subItem === 'string' ? subItem : subItem.name;
            const key = `${checklist.id}_${name}`; // [FIX] Name-based key — consistent with state
            const subDetail = (formData as any).subItemDetails?.[key];

            if (checklist.type === 'BOOLEAN') {
              if (!subDetail || subDetail.isPass === undefined || subDetail.isPass === null) {
                missingFields.push(`${checklist.topic} (${name})`);
              }
            } else {
              // NUMERIC, TEXT, DROPDOWN
              if (!subDetail || !subDetail.value || subDetail.value.toString().trim() === "") {
                missingFields.push(`${checklist.topic} (${name})`);
              }
            }
          }
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        // Calculate status based on Due Date (nextPMDate from plan or saved dueDate)
        ...
        if (targetDueDate) {
          ...
          // Fallback logic for legacy records that don't have a saved "dueDate"
          // We freeze the original status to prevent it from recalculating against the wrong future plan
        ...
        const payload = {
          ...
          details: formData.details
            .filter((d, index) => {
              // [FIX] Use global checklists to map details accurately. Details map 1:1 with `checklists` state.
            ...
            .map((d) => {
              ...
              const cl = checklists.find((c: any) => c.id === d.checklistId);
              return {
                ...d,
                topic: cl?.topic || "",
                checklistId: d.checklistId
  ...
  const renderSubItemSection = () => {
    ...
    subItemChecklists.forEach((checklist: any) => {
      ...
      // [FIX] Deduplicate subItems using common helper to align keys
      rawSubItems = getDedupedSubItems(rawSubItems);
                    ...
                    <thead className="table-light">
                      <tr>
                        ...
                        {subItemNames.map((si, idx) => {
                          ...
                          const hasSubSpec = siMin !== undefined || siMax !== undefined;
                          return (
                        ...
                        return (
                          <tr key={checklist.id}>
                            ...
                            {subItemNames.map((si, idx) => {
                              ...
                              const lastVal = lastPMValues[`${checklist.id}_${name}`];
                              return (
                                <td key={idx} className="text-center align-middle">
                                  <div className="text-danger" style={{ fontSize: '0.6rem' }}>RMin: {resolvedMin}, RMax: {resolvedMax}, hasSpec: {String(hasSpec)}</div>
                                  {checklist.type === 'NUMERIC' ? (
                                    <div>
                                      ...
                                      {/* [FIX] Show last PM value for NUMERIC */}
                                      {!isViewMode && !isEditMode && lastVal !== undefined && lastVal !== null && (
                                        <small className="text-info d-block fw-bold" style={{ fontSize: '0.7rem', marginTop: '2px' }}>
                                          Last PM: {lastVal}
                                        </small>
                                      )}
                                    </div>
                                    ...
                                    <div>
                                      <select
                                        ...
                                        onChange={(e) => {
                                          ...
                                          const dropdownOpts = parseDropdownOptions(checklist.options);
                                          const isPass = validateDropdownValue(val, dropdownOpts, effMin, effMax);
                                      ...
                                      {/* DROPDOWN Pass/Fail UI */}
                                      {subDetail.value && subDetail.isPass === false && <span className="badge bg-danger mt-1" style={{ fontSize: '0.6rem' }}>NG</span>}
                                      {subDetail.value && subDetail.isPass === true && <span className="badge bg-success mt-1" style={{ fontSize: '0.6rem' }}>OK</span>}
                                      {/* [FIX] Show last PM value for DROPDOWN */}
                                      {!isViewMode && !isEditMode && lastVal !== undefined && lastVal !== null && (
                                        <small className="text-info d-block fw-bold" style={{ fontSize: '0.7rem', marginTop: '2px' }}>
                                          Last PM: {lastVal}
                                        </small>
                                    ...
                                    <div>
                                      <input
                                        ...
                                        disabled={isViewMode}
                                      />
                                      {/* [FIX] Show last PM value for TEXT */}
                                      {!isViewMode && !isEditMode && lastVal !== undefined && lastVal !== null && (
                                        <small className="text-info d-block fw-bold" style={{ fontSize: '0.7rem', marginTop: '2px' }}>
                                          Last PM: {lastVal}
                                        </small>
                                    ...
                                    <div>
                                      ...
                                      {/* [FIX] Show last PM value for BOOLEAN */}
                                      {!isViewMode && !isEditMode && lastVal !== undefined && lastVal !== null && (
                                        <small className="text-info d-block fw-bold mt-1" style={{ fontSize: '0.7rem' }}>
                                          Last PM: {lastVal}
                                        </small>
                                      )}
                                    </div>
