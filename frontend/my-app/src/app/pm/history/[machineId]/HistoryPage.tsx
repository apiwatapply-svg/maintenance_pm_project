...
export default function PMHistoryPage() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            axios.get(`${config.apiServer}/api/machines/dropdown`),     // [FIX] Use lightweight endpoint
    ...
    const getDisplayValue = (record: PMRecord, topic: string, isActualSubItem: boolean = false) => {
        ...
        if (type === 'DROPDOWN') {
            ...
            if (master) {
                ...
                // [FIX] For sub-items, use per-sub-item spec only (don't fallback to parent)
                    ...
                    try {
                        ...
                        if (parsed.subItems && Array.isArray(parsed.subItems)) {
                            ...
                            if (matchedSI && typeof matchedSI !== 'string') {
                                ...
                                // String sub-item — keep parent's min/max if available (don't clear)
                            }
                        }
                    } catch { }
                }
